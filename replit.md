...
## Recent Changes
- **November 11, 2025**: Completed email verification system for new user signups - SHA-256 hashed verification tokens with 24-hour expiry and database-level validation. Signup flow updated to send verification email instead of auto-login (blocks unverified users at login with clear error message, admins exempted). Frontend pages: /auth/verify-email/:token with auto-login and dashboard redirect. Backend routes: POST /api/auth/signup (sends email), GET /api/auth/verify-email/:token (verifies token, creates session, returns user). Email service utility (server/lib/email.ts) supports SMTP and console mode. Production-ready minus SMTP configuration (EMAIL_HOST, EMAIL_USER, EMAIL_PASS, EMAIL_FROM environment variables).
- **November 11, 2025**: Completed password reset system with secure token-based workflow - SHA-256 hashed tokens, timing-safe comparisons, sliding-window rate limiting (3/hour forgot, 5/hour reset, 10/min verify), CSRF protection, generic responses to prevent email enumeration, one-time use enforcement, 24-hour expiry, session invalidation on reset. Frontend pages: /auth/forgot-password and /auth/reset-password/:token. Backend routes: POST /api/auth/forgot-password, GET /api/auth/verify-reset-token/:token, POST /api/auth/reset-password. Fixed critical bug: root admin now has userId='PB1' (was missing). Production-ready minus email service integration (currently logs to console).
- **November 11, 2025**: Implemented receiverType-based payment confirmation system - payments are now confirmed exclusively by their designated receiver. If receiverType='user', only that specific user can confirm/reject (admin cannot interfere). If receiverType='admin', only admin can confirm/reject (users cannot interfere). Updated storage queries (getActivationPaymentsPendingConfirmation, getAdminPendingConfirmations) and API routes (/confirm, /reject) to enforce strict receiverType authorization.
